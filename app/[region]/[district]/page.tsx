...
// 💡 서버 컴포넌트에서 동작하여 검색엔진 로봇이 지역명과 출장마사지 키워드를 완벽하게 수집함
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const titleVariants = [
    `${locationKeyword} 출장마사지 추천 | 24시 빠른방문 힐링 케er - 휴식의정원`,
    `[휴식의정원] ${locationKeyword} 출장마사지 안내 · 100% 안심 후불제`,
    ...
    `휴식의정원 | ${simpleLocation} 출장마사지 안심 후불제 힐링 테라피`,
    ...
    `[24시 방문] ${locationKeyword} 출장마사지 추천 업체 모음 · 휴식의정원`,
    `${simpleLocation} 출장마사지 릴렉싱 케어 | 후불제 홈테라피 가이드`,
    `${locationKeyword} 출장마사지 25분 내 빠르게 달려갑니다 - 휴식의정원`,
    ...
    `${locationKeyword} 출장마사지 신속한 예약 서비스 | 휴식의정원 공식`,
    `[휴식의정원 24시] ${locationKeyword} 출장마사지 베테랑 힐러 케어`,
  ...
  const descriptionVariants = [
    ...
    `${locationKeyword} 전지역 신속 출장마사지 예약. 부담 없는 후불제 시스템과 정직한 코스 정보 제공, 휴식의정원 공식 안내.`,
    ...
    `휴식의정원에서 보장하는 ${locationKeyword} 출장마사지 안심 서비스! 선입금 요구 없이 도착 후 결제하는 100% 안전 시스템.`,
  ...
  return {
    ...
    openGraph: {
      title: finalTitle,
      description: finalDescription,
      url: `https://Jungwon-healing.netlify.app/${region}/${encodeURIComponent(districtName)}${dongName ? `?dong=${encodeURIComponent(dongName)}` : ""}`,
      siteName: "휴식의정원",
      locale: "ko_KR",
      type: "website",
    },
